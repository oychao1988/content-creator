# 文档导航

本文档提供所有项目文档的分类导航。

---

## 📚 文档分类

### 🚀 快速开始
- [快速开始指南](./quick-start.md) - 5分钟上手指南
- [用户指南](./user-guide.md) - 详细使用说明
- [CLI 参考手册](./cli-reference.md) - 命令行工具完整参考

### 🏗️ 架构文档
- [完整架构文档](./architecture-complete.md) - 系统整体架构
- [工作流架构](./workflow-architecture.md) - 工作流系统设计
- [质检架构](./quality-check-architecture.md) - 质量检查系统
- [存储指南](./storage-guide.md) - 存储服务使用

### 🔧 配置相关
- [配置系统更新](./configuration/config-system-update.md) - 配置系统说明

### 📖 操作指南
- [工作流扩展指南](./workflow-extension-guide.md) - 如何扩展工作流
- [翻译工作流指南](./guides/translation-workflow-guide.md) - 翻译工作流使用
- [工作流适配器使用](./guides/workflow-adapter-usage.md) - 适配器模式说明

### 🔄 工作流扩展
- [扩展设计文档](./workflow-extension-design.md) - 扩展架构设计
- [扩展实现总结](./workflow/workflow-extension-SUMMARY.md) - 实现总结
- [设计对比分析](./workflow/workflow-extension-COMPARISON.md) - 设计与实现对比
- [未来开发指南](./workflow/workflow-extension-FUTURE-GUIDE.md) - 扩展开发指南
- [扩展计划](./workflow/WORKFLOW-EXTENSION-PLAN.md) - 开发计划
- [扩展进度](./workflow/WORKFLOW-EXTENSION-PROGRESS.md) - 实施进度
- [架构阶段2](./workflow/workflow-architecture-stage2.md) - 第二阶段架构
- [架构阶段3](./workflow/workflow-architecture-stage3.md) - 第三阶段架构

### 🛠️ 开发相关
- [测试实现计划](./development/test-implementation-PLAN.md) - 测试实现计划
- [测试实现总结](./development/test-implementation-SUMMARY.md) - 测试实现总结
- [数据库重构计划](./development/database-refactoring-PLAN.md) - 数据库重构计划
- [数据库重构总结](./development/database-refactoring-SUMMARY.md) - 数据库重构总结

### 📊 项目报告
- [项目完成报告](./reports/PROJECT-COMPLETION-REPORT.md) - 项目完成总结
- [阶段4完成报告](./reports/STAGE-4-COMPLETION-REPORT.md) - 阶段4总结
- [阶段6完成报告](./reports/STAGE6-COMPLETION-REPORT.md) - 阶段6总结
- [测试修复报告](./reports/TEST-FIX-REPORT.md) - 测试问题修复
- [NPM 发布清单](./reports/NPM-PUBLISH-CHECKLIST.md) - 发布检查清单
- [超时测试报告](./timeout-test-report.md) - 超时配置测试

### 📚 参考资料
- [BullMQ 快速参考](./references/bullmq-quick-reference.md) - BullMQ 使用
- [性能优化指南](./references/performance-optimization-guide.md) - 性能优化
- [监控优化指南](./references/monitoring-optimization-guide.md) - 监控优化

### 🗄️ 归档文档
- [文档分析](./DOCUMENTATION-ANALYSIS.md) - 文档结构分析
- [源码分析](./SOURCE-CODE-ANALYSIS.md) - 代码结构分析
- [Agent 性能评估设计](./agent-performance-evaluation-design.md) - 性能评估

---

## 📂 按用途分类

### 给新用户
1. 📖 [快速开始](./quick-start.md)
2. 📖 [用户指南](./user-guide.md)
3. 📖 [CLI 参考手册](./cli-reference.md)

### 给开发者
1. 🏗️ [完整架构文档](./architecture-complete.md)
2. 🔧 [配置系统更新](./configuration/config-system-update.md)
3. 🛠️ [开发相关文档](./development/)
4. 📚 [参考资料](./references/)

### 给扩展开发者
1. 📖 [工作流扩展指南](./workflow-extension-guide.md)
2. 📖 [未来开发指南](./workflow/workflow-extension-FUTURE-GUIDE.md)
3. 📖 [工作流适配器使用](./guides/workflow-adapter-usage.md)

### 项目管理
1. 📊 [项目完成报告](./reports/PROJECT-COMPLETION-REPORT.md)
2. 📊 [各阶段完成报告](./reports/)
3. 📊 [工作流扩展计划](./workflow/WORKFLOW-EXTENSION-PLAN.md)

---

## 🔍 快速查找

### 我想要...

**快速上手项目**
→ 查看 [快速开始](./quick-start.md)

**了解系统架构**
→ 查看 [完整架构文档](./architecture-complete.md)

**使用命令行工具**
→ 查看 [CLI 参考手册](./cli-reference.md)

**扩展工作流**
→ 查看 [工作流扩展指南](./workflow-extension-guide.md)

**了解质量检查**
→ 查看 [质检架构](./quality-check-architecture.md)

**配置系统**
→ 查看 [配置系统更新](./configuration/config-system-update.md)

**查看测试相关**
→ 查看 [测试实现总结](./development/test-implementation-SUMMARY.md)

**了解项目进展**
→ 查看 [项目完成报告](./reports/PROJECT-COMPLETION-REPORT.md)

---

## 📝 文档维护

### 文档规范
- 所有文档使用 Markdown 格式
- 文件名使用 kebab-case（小写-连字符）
- 每个文档应有清晰的标题和目录
- 代码示例应包含必要的说明

### 文档分类规则
- **快速开始**: 入门和用户指南
- **架构**: 系统设计和架构文档
- **配置**: 环境配置和部署
- **指南**: 操作指南和教程
- **开发**: 开发计划和实现总结
- **报告**: 项目报告和测试报告
- **参考资料**: 技术参考文档
- **归档**: 历史文档和分析

### 更新文档
更新文档时请确保：
1. 更新相关的内容
2. 更新本导航页面的链接
3. 在文档顶部注明更新日期
4. 遵循现有的文档格式

---

## 🔗 相关链接

- [项目主页](../README.md)
- [更新日志](../CHANGELOG.md)
- [Claude 配置](../CLAUDE.md)
